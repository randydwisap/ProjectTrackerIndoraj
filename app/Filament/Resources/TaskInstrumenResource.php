...
use App\Filament\Resources\TaskInstrumenResource\Pages;
use App\Filament\Resources\TaskInstrumenResource\RelationManagers;
...
use App\Filament\Resources\TaskInstrumenResource\RelationManagers\ReportInstrumenRelationManagers;

class TaskInstrumenResource extends Resource
{
    protected static ?string $model = TaskInstrumen::class;

    protected static ?string $navigationIcon = 'heroicon-o-briefcase';
    protected static ?string $navigationLabel = 'Penyusunan Instrumen';
    protected static ?string $navigationGroup = 'Lainnya';
    protected static ?string $pluralLabel = 'Penyusunan Instrumen';
    protected static ?int $navigationSort = 2; // Menentukan urutan menu

         public static function getRelations(): array
     {
         return [
             ReportInstrumenRelationManagers::class,
        ];
     }
    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                ...
                ->options(function ($get, $record) {
                    $query = Marketing::query()
                        ->where('jenis_pekerjaan', 'Penyusunan Instrumen')
                ...
                ->afterStateUpdated(function ($state, callable $set, callable $get) {
                    ...
                    if ($marketing) {
                        ...
                        $set('tahap_pengerjaan', 'Wawancara');
            ...
            Forms\Components\Select::make('tahap_pengerjaan')
                ...
                ->required()
                ->options(\App\Models\JenisInstrumen::pluck('nama_task', 'nama_task'))
                ->default('Wawancara'),
        ...
        return $table
            ...
            ->filters([
                SelectFilter::make('lokasi')
                    ->label('Filter Lokasi')
                    ->searchable()
                    ->options(fn () => TaskInstrumen::query()->distinct()->pluck('lokasi', 'lokasi')->toArray()),
                SelectFilter::make('pekerjaan')
                    ->label('Filter Pekerjaan')
                    ->searchable()
                    ->options(fn () => TaskInstrumen::query()->distinct()->pluck('pekerjaan', 'pekerjaan')->toArray()),
                SelectFilter::make('tahap_pengerjaan')
                    ->label('Filter Tahap')
                    ->searchable()
                    ->options(fn () => TaskInstrumen::query()->distinct()->pluck('tahap_pengerjaan', 'tahap_pengerjaan')->toArray()),
    ...
    {
        return [
            'index' => Pages\ListTaskInstrumens::route('/'),
            'create' => Pages\CreateTaskInstrumen::route('/create'),
            'edit' => Pages\EditTaskInstrumen::route('/{record}/edit'),
        ];
    }
